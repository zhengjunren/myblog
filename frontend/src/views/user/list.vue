<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="头像" prop="email"  align="center" width="100">
      ...
      <el-table-column label="用户首页" prop="url" align="center" width="200">
        <template slot-scope="{row}">
          <el-link v-bind:href="'http://'+row.url" target="_blank" type="primary">{{row.url}}</el-link>
        </template>
      </el-table-column>
      <el-table-column label="上次登录时间" prop="lastLoginTime" align="center" width="200">
        <template slot-scope="scope">
          <span>{{ scope.row.lastLoginTime }}</span>
        </template>
      </el-table-column>
      <el-table-column label="用户状态" class-name="status-col" align="center" width="100">
        <template slot-scope="{row}">
      ...
      <el-table-column label="操作" align="center" width="280" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          ...
          <el-button v-if="row.status!== '正常'" size="mini" type="success" >
            正常
          </el-button>
          <el-button v-if="row.status!== '冻结'" size="mini" type="warning">
            冻结
          </el-button>
          <el-button v-if="row.status!== '注销'" size="mini" type="danger">
            注销
          </el-button>
        </template>
      </el-table-column>
